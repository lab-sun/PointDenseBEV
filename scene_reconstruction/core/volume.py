...
class Volume:
    ...
    def _grid_sample_from_self(self) -> Tensor:
        ...
        # print("self.lower, self.upper",self.lower, self.upper)
        return transform_to_grid_sample_coords(self.lower, self.upper) # [[-25.0000, -50.0000,  -2.5000]],[[25.0000, 50.0000,  1.5000]]
    ...
    def new_coord_grid(self, volume_shape: Sequence[int]) -> Tensor:
        ...
        assert len(volume_shape) == 3 # 1d tensor [1000, 500, 8]
        ...
        # 对原始网格增加一维，[1, 1000, 500, 8]->[1, 1000, 500, 8, 3] 3表示每个体素中心点xyz坐标
        return grid
    ...
    def sample_volume(
        ...
        cartesian_rt = sph_volume.sample_volume(volume_features=spherical_rt, sample_coords=sample_points_spherical) 
        """
        # volume_features: [1, 2, 600, 100, 720] 
        # sample_coords: [B, C,1000，500， r θ ϕ] 极坐标
        normalized_from_grid = self._grid_sample_from_self() # 归一化变换矩阵 [1, 4, 4]
        # print("normalized_from_grid",normalized_from_grid)
        normalized_coords = einsum_transform("bng,bxyzg->bxyzn", normalized_from_grid, points=sample_coords) # [1, 1000, 500, 8, 3]
        normalized_coords = normalized_coords.expand(volume_features.shape[0], -1, -1, -1, -1) # [1, 1000, 500, 8, 3]
        # kitti 对应的normalized_coords超出（-1，1）范围
        return torch.nn.functional.grid_sample(
            volume_features,   # [1, 2, 600, 100, 720]
            normalized_coords, # [1, 1000, 500, 32, 3]
            mode=mode, 
            align_corners=False
