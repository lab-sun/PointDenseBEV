...
def transform_volume_bounds(
    lower: Tensor,      # [-50.0000, -25.0000,  -2.5000]
    upper: Tensor,      # [50.0000, 25.0000,  1.5000]
    new_lower: Tensor,  # -torch.ones_like(lower)
    new_upper: Tensor,  # torch.ones_like(upper)
    verbose: bool = False,
):
...
def transform_to_grid_sample_coords(lower, upper):
    ...
    # swap_transform = torch.eye(4, device=lower.device, dtype=lower.dtype)
...
def einsum_transform(einsum_str: str, *transform: Tensor, points: Tensor):
    ...
    is_homogenous = points.shape[-1] == 4 # point [1, 1000, 500, 8, 3] 检查输入点是否已经是齐次坐标
