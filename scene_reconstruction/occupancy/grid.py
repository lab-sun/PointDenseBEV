...
def volume_density_from_points(
    volume_shape: Sequence[int], # [601, 100, 720]
    volume: Volume, # lower([[ 2.0000,  1.3090, -3.1416]], upper=tensor([[60.0967,  2.1817,  3.1416]]
    points: Tensor, # [1, 124823, 3]
    points_weights: Optional[Tensor] = None, # [1, 124823, 1]
    ...
    prod_volume_shape = reduce(operator.mul, volume_shape, 1)
    # features_flat = torch.zeros(batch_size, num_channels, math.prod(volume_shape), device=points.device)
    features_flat = torch.zeros(batch_size, num_channels, prod_volume_shape, device=points.device)
...
def spherical_reflection_and_transmission_from_lidar(
    ...
    # add one bin along r
    spherical_shape_with_inf = [spherical_shape[0] + 1, spherical_shape[1], spherical_shape[2]] # [601, 100, 720]
    ...
    ) # Volume(lower=tensor([[ 2.0000,  1.3090, -3.1416]], device='cuda:0'), upper=tensor([[60.0967,  2.1817,  3.1416]], device='cuda:0'))

    reflection_and_transmission = volume_density_from_points(
        volume_shape=spherical_shape_with_inf, # [601, 100, 720]
        volume=spherical_volume_with_inf, # lower([[ 2.0000,  1.3090, -3.1416]], upper=tensor([[60.0967,  2.1817,  3.1416]]
        points=points_lidar_spherical, # [1, 124823, 3]
        points_weights=points_weight, #[1, 124823, 1] 全1
        modes=(("drop", "clamp"), "clamp", "warp"),  # clamp upper range, drop lower range
    )
    # print("reflection_and_transmission",reflection_and_transmission[0,0,0,0])
    num_reflections = reflection_and_transmission # [1, 1, 601, 100, 720]
    num_transmissions = reflection_and_transmission.sum(2, keepdim=True) - reflection_and_transmission.cumsum(2) # [1, 1, 601, 100, 720]

    # drop infinity bin
    reflection_and_transmission = torch.cat(
        [num_reflections[:, :, :-1, :, :], num_transmissions[:, :, :-1, :, :]], 1
    )  # [B, C, R, PHI, THETA] # torch.Size([4, 2, 600, 100, 720])
    if normalize == "volume":
