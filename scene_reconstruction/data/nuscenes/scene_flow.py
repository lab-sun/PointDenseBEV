...
def extract_instance_ids_chunked(
    ...
    **kwargs,
    ...
    grid_points = ego_volume.to(**kwargs).new_coord_grid(volume_shape) # 生成一个 3D 坐标网格
    # chunk along x dim
...
class SceneFlow:
    ...
    def process_scene(self, scene_name: str):
        ...
        # scene_instance_index (idex, token)
        ...
        volume_lower = torch_to_series("LIDAR_TOP.scene_flow.volume.lower", ego_volume.lower) # Series: 'LIDAR_TOP.scene_flow.volume.lower' [array[f32, 3]]
        volume_upper = torch_to_series("LIDAR_TOP.scene_flow.volume.upper", ego_volume.upper) # Series: 'LIDAR_TOP.scene_flow.volume.upper' [array[f32, 3]]
            ...
            if sample["LIDAR_TOP.sample_data.token"].is_in(scene_with_annos["LIDAR_TOP.sample_data.token"]).item():
                ...
                sample_with_ids = sample.select("LIDAR_TOP.sample_data.token").with_columns(
                    torch_to_series(
                        ...
                        torch.zeros(1, *self.cartesian_shape, dtype=torch.long), # 按照cartesian_shape建立全0矩阵
                    )
                )
            sample = self.ds.join(
                sample.select(
