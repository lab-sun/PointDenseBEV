...
class NuscenesDataset:
    ...
    def read_json(self, name: ANNOTATION_FILES):
        ...
        # print(self.data_root / self.version / f"{name}.json")
        # print(name)
    ...
    def _load_sensor_data(
        ...
        if sensor_channel in get_args(CAMERA_CHANNELS):
            ...
            prefix = f"{sensor_channel}.sample_data"
            filename = f"{sensor_channel}.sample_data.filename" # 从sample和swqqp读取了连续4帧lidar
        ...
        # print(np.shape(list_of_dicts[0]["points_mask"])) 
        # list ['points_lidar','points_mask'] 4len (35000, 3) (35000)
        dict_of_data = {f"{prefix}.{k}": np.stack([d[k] for d in list_of_dicts]) for k in list_of_dicts[0].keys()} 
        # print(np.shape(dict_of_data['LIDAR_TOP.sample_data.points_mask']))  
        # dic{'LIDAR_TOP.sample_data.points_lidar','LIDAR_TOP.sample_data.points_mask'}(4, 35000, 3),(4, 35000)
