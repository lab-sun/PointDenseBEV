...
class DataProcessor(object):
    ...
    def image_normalize(self, data_dict=None, config=None):
        # 使用 torchvision 将图像转换为 Tensor，并进行归一化
        if data_dict is None:
    ...
    def image_crop(self, data_dict=None, config=None):
        '''
        作用: 图像裁剪。
        训练时: 执行随机裁剪（Random Crop），增强数据多样性。
        测试时: 执行中心裁剪（Center Crop）。
        记录裁剪信息 img_process_infos（包含 resize比例, crop坐标, flip, rotate 等），用于 2D 变换逆推回 3D 
        '''
        if data_dict is None:
    ...
    def forward(self, data_dict):
        ...
        Returns:
        """
        # print("[DEBUG] data_processor_queue: ", self.data_processor_queue)
