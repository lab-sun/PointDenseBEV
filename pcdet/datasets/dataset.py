...
class DatasetTemplate(torch_data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # occu_map, visb_map = self.map_generator.gene_map(result_reftrans) # [500, 1000, 1], [500, 1000, 1] 对齐(500, 1000, 1)即可

        # 3、加载点云对应标签
        seg_gt = self.get_dense_gt_lab(seq, idx)  # f_file = self.gt_dense_img_root / seq / ('%s.png' % index) 
        # seg_gt = self.get_sparse_gt_lab(seq, idx)  # sparse数据
        data_dict['labels_seg'] = seg_gt
        # 读取点云对应的可见性地图[0]和二值地图[1]
        obser =  self.get_obser_bin(seq, idx)
        # obser =  np.ones((2,500, 1000,1)) # 暂时用全0填充
        data_dict['observations'] = obser[0] # (500, 1000, 1)
        data_dict['binarymap'] = obser[1] # (500, 1000, 1) ndarray

        data_dict['reflections'] = ref_trans[0, 0].numpy() # [500, 1000, 4] Tensor
        data_dict['transmissions'] = ref_trans[0, 1].numpy() # [500, 1000, 4]
    ...
    def prepare_data(self, data_dict):
        ...
        # data_dict = self.point_feature_encoder.forward(data_dict) # 用于确定使用点的哪些属性 比如x,y,z等 kitti数据集不需要
        # data_processor对点云进行预处理，包括移除超出point_cloud_range的点、 打乱点的顺序以及将点云转换为voxel
