...
def eval_one_epoch(cfg, model, dataloader, epoch_id, logger, dist_test=False, save_to_file=False, result_dir=None):
    ...
    with tqdm.tqdm(total=n_val, desc='Val', unit='batch', leave=False) as pbar:
        ...
        for i, batch_dict in enumerate(dataloader):
            ...
            pred = pred_dict["prediction"][:, :12, :, :] # torch.Size([1, 12, 500, 1000])
            ...
            pred_img = batch_dict['fullprediction'][:, :12, :, :] # fullprediction
        ...
        miou = ((sum(iou)) / (len(iou)))
        iou = torch.Tensor(iou)
        mlatency = ((sum(mlatency)) / (len(mlatency)))
        print('mlatency:',mlatency)
