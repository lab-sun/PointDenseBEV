...
def annealing_liner(start, end, pct):
    # print(pct, start, end)
...
class OneCycle(LRSchedulerStep):
    def __init__(self, fai_optimizer, total_step, lr_max, moms, div_factor, pct_start):
        ...
        self.pct_start = pct_start # # 0.1 学习率上升部分
        a1 = int(total_step * self.pct_start)
        a2 = total_step - a1
        low_lr = self.lr_max / self.div_factor # 初始学习率= max_lr / div_factor
...
class OneCycle_with_WarmupLR(LRSchedulerStep):
    def __init__(self, fai_optimizer, total_step, lr_max, moms, div_factor, pct_start):
        self.lr_max = lr_max
        self.moms = moms
        self.div_factor = div_factor
        ...
        lr_phases = ((0, partial(annealing_liner, low_lr, self.lr_max)),
